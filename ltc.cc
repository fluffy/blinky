...
class TimeCode {
 public:
  TimeCode() { valid=0; };
  TimeCode(uint32_t s, uint32_t us ) { frame=(us*30l/1000000l)%30; sec=s%60; min=(s/60)%60; hour=(s/3600)%24; valid=1; };
...
class LTC {
 public:
  LTC() { valid=0; };
  LTC( const TimeCode& time) { set(time); valid=1; };
...
void LTC::decode( const TransitionSet& tSet ){
  ...
  for ( int i=1; i< tSet.size; i++ ) {
    ...
    if (  ( delta > 700 ) && ( delta < 900 ) ) {
      ...
      bitCount++;
      if ( bitCount >= 8 ) { bitCount=0; byteCount++; }
      if ( byteCount >= 10 ) { return; };
      
    } else  if (  ( delta > 300 ) && ( delta < 500 ) ) {
      // start of 1
      if ( i+1 >=  tSet.size ) { return; }
  ...
  if  ( parity() != 0 ) { return; }
  if  (  bits[8] != 0x3F ) { return; }
  if  (  bits[9] != 0xFE ) { return; }
  valid = 1;
}
